...
Создание платежа, `POST /api/v1/payments`. Заголовок `Idempotency-Key` обязателен:
...
Consumer берёт сообщение, эмулирует обработку (2-5 секунд, около 90% успеха), проставляет статус
